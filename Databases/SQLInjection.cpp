...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
    ...
    // Convert the SQL query to lowercase to make detection simpler
    std::string lower_sql = sql;
    std::transform(lower_sql.begin(), lower_sql.end(), lower_sql.begin(), ::tolower);

    // Check for patterns like " or value=value" to detect potential SQL injection
    if (lower_sql.find(" or ") != std::string::npos &&
        (lower_sql.find("=1") != std::string::npos || lower_sql.find("='") != std::string::npos))
    {
        // If suspicious pattern is found, print a warning and prevent the query from running
        std::cout << "Potential SQL injection detected! Query blocked." << std::endl;
