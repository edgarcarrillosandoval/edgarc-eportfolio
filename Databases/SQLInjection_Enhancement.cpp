...
#include <tuple>
#include <vector>
#include <fstream> // Required for logging to a file
...
// ENHANCEMENT 3: Logging function to track suspicious query attempts
...
{
    // ENHANCEMENT: Use parameterized statements to prevent SQL injection
    // This avoids manually constructing SQL strings and directly blocks injection

    // ENHANCEMENT 2: Basic input validation before preparing query
    if (sql.empty()) {
        std::cout << "Invalid query: input is empty." << std::endl;
        return false;
    }
    ...
    if (lowered.find("--") != std::string::npos ||
        ...
        lowered.find("'") != std::string::npos ||
        lowered.find(";") != std::string::npos ||
        lowered.find(" or ") != std::string::npos) {
        std::cout << "Invalid query: input contains suspicious patterns." << std::endl;

        // ENHANCEMENT 3: Log the suspicious pattern
        log_event("Suspicious query blocked: " + sql);
    ...
    // Check if query ended properly
    if (rc != SQLITE_DONE) {
        std::cout << "Statement did not finish successfully. ERROR = " << sqlite3_errmsg(db) << std::endl;
